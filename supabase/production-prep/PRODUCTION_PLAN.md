...
| 3 | **Approve** | ✅ **done 2026-09-19** — the owner approved deleting all 233 (216 DELETE + 17 HOLD) and keeping the owner's auth user, profile and the bucket + policies | — |
| 4 | **Generate** the final cleanup SQL from the rehearsed order (§15.6) with a precondition (exact counts + id fingerprints equal the approved snapshot ⇒ detects drift) and post-conditions | ✅ **generated 2026-09-19, NOT executed** — `cleanup/08a` (dry run) and `cleanup/08b` (execute); see §15.11 | dress-rehearsed: 20/20 checks |
...
**✅ DECISION RECORDED (owner, 2026-09-19): option 1 approved — all 17 HOLD items are approved for deletion (216 + 17 = 233), the owner's auth user + profile and the bucket + policies are kept, nothing outside the approved classification is deleted.**
Options that were considered: (1) **recommended — approve deleting all 17 HOLD rows** (all test artifacts): **233 DELETE + 3 KEEP**, the database ends with only the owner's account; (2) keep some/all HOLD rows and accept the
...
Nothing has been deleted yet: the cleanup SQL is generated but **not executed** (§15.11).
...
### 15.11 The cleanup SQL — generated 2026-09-19, **NOT executed** (`supabase/production-prep/cleanup/`)
Generated by `tools/generate_cleanup_sql.mjs` from inventories 06 + 07 and the approved classification (it refuses to generate unless the totals are exactly DELETE 216 / HOLD 17 / KEEP 3 / unresolved 0).
The SQL contains only ids, counts and md5 fingerprints — no e-mails, titles or comment text.

| file | what it is |
|---|---|
| `08a_cleanup_DRY_RUN.sql` | the same guard, precondition, deletes and post-checks as 08b, but **ends in `ROLLBACK`** — proves the whole run on the real database without keeping anything |
| `08b_cleanup_EXECUTE.sql` | identical, but **ends in `COMMIT`** — the real cleanup |
| `09_DASHBOARD_STEPS.md` | the 2 storage files and the 3 test auth users, deleted in the Dashboard **after** 08b |
| `10_post_dashboard_verify_readonly.sql` | one read-only `SELECT` that must return `"all_ok": true` at the end |

**What 08b deletes (225 rows in 26 tables, one transaction):** every row of `audit_logs`, `cad_review_comments`, `cad_review_versions`, `cad_reviews`, `calendar_events`, `comment_mentions`, `member_applications`,
`migration_exceptions`, `notifications`, `purchase_request_items`, `purchase_status_history`, `purchase_requests`, `subsystem_members`, `task_assignees`, `task_attachments`, `task_comments`, `task_requests`, `tasks`,
`timeline_milestones`, `milestones`, `recurring_events`, `subsystem_categories`, `subsystems`, `timeline_columns`, `competition_settings`, `migration_log` — in that (children-first) order. It does **not** touch `profiles`,
`auth.users`, `storage.objects`, the bucket, the schema, policies, functions, triggers, grants or Auth settings. The other **8** approved items are removed in the Dashboard: 3 test auth users (their 3 profiles cascade) and 2 storage files
(**225 + 8 = 233**, the generator asserts this).

**How it cannot delete anything outside the approved list:**
1. **Guard** — aborts if the legacy tables (`workspace_state`, `orders`, `subteams`) exist (i.e. pasted into the OLD project by mistake), if the v2 tables are missing, or if the owner account is not in `auth.users`.
2. **Locks** the 26 tables against writes (reads still work) until the transaction ends, so nothing can change between the check and the deletes.
3. **Exact-count precondition + id fingerprint:** for **29 tables** (the 26 to empty + `profiles`, `auth.users`, `storage.objects`, which must stay unchanged) it compares the live row count **and an md5 of the sorted row keys** with the approved
   inventory. One extra, missing or *different* row (even with the same count) aborts before any delete.
4. **Deletes** with no `WHERE` (safe *because* step 3 proved the tables hold exactly the approved rows).
5. **Post-checks inside the same transaction** (any failure rolls everything back): the 26 tables are empty; `profiles` = 4 and the owner is still `admin`/approved/active; `auth.users` = 4; `storage.objects` = 2; the bucket
   (private, 20 MiB) and its 2 policies are intact; the schema is untouched — 27 tables, 88 policies, 38 public triggers, 36 public functions, every public/`auth.users` trigger enabled, `on_auth_user_created` present.

**Run order:** 08a (dry run) → 08b (execute) → Dashboard step A (storage files) → Dashboard step B (3 users) → `10_…` must return `all_ok: true`. The SQL Editor may ask for confirmation because the script contains `DELETE`.

**Dress rehearsal (`tools/rehearse_cleanup_sql.mjs`, scratch Postgres, never Supabase):** a database with migrations 0001–0021 loaded with the **real row ids** of all 229 rows (foreign keys enforced) — **20/20 checks passed**:
the fixture equals the inventory; the dry run passes and changes nothing; execute deletes exactly 225 rows, leaves `profiles`/`auth.users`/`storage.objects` at 4/4/2; after the simulated Dashboard steps `10_…` returns `all_ok: true`
(1 auth user, 1 profile, 0 objects, bucket + 2 policies); **8 negative tests each abort and leave every row in place** — an extra row, a missing row, the *same count with one different id*, a legacy table present, the owner not `admin`,
...
### 16.2 Import design (bobcat-dev): `import/40_import_TEMPLATE_DO_NOT_RUN.sql` (a template — never paste it) → generated `results/import/40a_import_DRY_RUN.sql` / `40b_import_EXECUTE.sql` (the only files to run)
...
### 16.4 Rehearsal evidence (`tools/rehearse_import.mjs`, scratch Postgres, **never Supabase**) — **48/48 checks** (the original 39 + 9 generator-hardening checks, §16.5b)
...
### 16.5b Incident and fix — "syntax error at or near `__MODE_END__`" (2026-09-20)
**Symptom:** the bobcat-dev dry run failed instantly with `ERROR 42601: syntax error at or near "__MODE_END__"  LINE 363`. Nothing was imported (PostgreSQL parses a whole pasted script before executing any statement).
**Root cause (proven, not assumed):** the generator did **not** leak a marker — the generated `40a_import_DRY_RUN.sql` on disk had 0 placeholders and ended in `rollback;`. The text that reached the editor was the **template** (`import/40_import_template.sql`): it is exactly **363 lines**, its last line (363) is the bare `__MODE_END__`, and feeding it to the PostgreSQL parser reproduces the identical error. The two files were easy to confuse — similar names, both plain `.sql`, and the template failed only on its very last line with a cryptic message.
**Fix (template + generator level, nothing was hand-edited):**
1. The template is renamed **`40_import_TEMPLATE_DO_NOT_RUN.sql`** and is now parse-safe: every marker sits inside a comment or a string, and its first statement raises `THIS IS THE TEMPLATE … Run the GENERATED file instead: results/import/40a_import_DRY_RUN.sql. Nothing was changed.`
2. The generator strips the whole template banner + guard from its output, fills placeholders with plain string replacement (a `$&`, `$'`, `$1` or even marker-like text inside legacy data can never be interpreted or re-scanned; the payload is inserted last, by index), stamps each file (`-- GENERATED …` first line, `-- END OF GENERATED SCRIPT …` last line, so a truncated paste is visible), and **runs 38 static checks before leaving any file behind**. On any failure it exits non-zero and deletes its outputs, including stale files from earlier runs — a failed generation can never leave a half-good file.
3. `tools/check_generated_import_sql.mjs` (shared module `tools/import_sql_checks.mjs`) re-runs the static checks — 50 including the expected counts — on the exact files you are about to paste.
**Verification:** 50/50 static checks; the template and both generated files all parse under the real PostgreSQL parser; the rehearsal is **48/48** (the original 39 + 9 new: the template refuses clearly; the original defect, a stray marker, an unstrippable guard and two importer ids are each refused with no file left; `$& $' $1 $$` and `__PAYLOAD__ __EXPECT__ __IMPORTER__` inside data survive byte-for-byte; data containing the literal `__MODE_END__` is refused); the real-export scratch rehearsal passes dry run + execute (63 tasks, 6 held, 29 exceptions, 136 log rows, 10/10 overlapping ids from `workspace_state`, owner profile unchanged, `41_` all_ok). The payload md5 is unchanged (`424a5a23b28f7a60e3ab930b299c0c25`).
...
3. I generate `results/import/40a_import_DRY_RUN.sql` and `40b_import_EXECUTE.sql` from the validated export and show you the summary (row counts, held tasks, exceptions). **Before pasting, run `node supabase/production-prep/tools/check_generated_import_sql.mjs` (must say ALL PASSED) and paste ONLY the generated file from `results/import/` — never the template.**
